...
class SGRecall(SceneGraphEvaluation):
    def __init__(self, config, result_dict):
        ...
        self.ru_net = config.MODEL.ROI_RELATION_HEAD.PREDICTOR == "RUNetPredictor"
        self.hl_net = config.MODEL.ROI_RELATION_HEAD.PREDICTOR == "HLNetPredictor"
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # RU-Net
        """
        if self.ru_net or self.hl_net:
            if self.hl_net:
                self.l21_thr = 0.7
            if self.l21_thr is not None:
        """
        if self.cfg.DATASETS.POST_NMS:
            self.l21_thr = 0.7
            pred_rels, pred_scores = rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores, 0.5, self.l21_thr)
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        ...
        result_str += '\n'
        return result_str, self.match_container
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        self.gt_triplets = np.column_stack((gt_classes[sub_id], gt_classes[ob_id], pred_label))  # num_rel, 3

        self.zeroshot_idx = np.where( intersect_2d(self.gt_triplets, zeroshot_triplets).sum(-1) > 0 )[0].tolist()

    def calculate_recall(self, img_id, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_zeroshot_recall']:
            ...
            if len(self.zeroshot_idx) > 0:
                ...
                if k == 100:
                    match_idx = set(self.zeroshot_idx).intersection(set(match_list))
                    img_contain = [str(img_id)]
                    for idx in match_idx:
                        idx = int(idx)
                        img_contain.append(self.gt_triplets[idx])
                    self.match_container.append(img_contain)
                        # except:
                        #     print(match_idx, len(self.match_container))
                        #     print(idx, self.gt_triplets.shape, self.gt_triplets)
...
class SGZeroShotMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        super(SGZeroShotMeanRecall, self).__init__(result_dict)
        self.num_rel = num_rel
        self.print_detail = print_detail
        self.rel_name_list = ind_to_predicates[1:] # remove __background__

    def register_container(self, mode):
        self.result_dict[mode + '_mean_zs_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_mean_recall_zs_collect'] = {20: [[] for i in range(self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode + '_mean_recall_zs_list'] = {20: [], 50: [], 100: []}

    def generate_print_string(self, mode):
        result_str = 'SGG eval: '
        for k, v in self.result_dict[mode + '_mean_zs_recall'].items():
            result_str += 'zs-mR @ %d: %.4f; ' % (k, float(v))
        result_str += ' for mode=%s, type=Zero-Shot Mean Recall.' % mode
        result_str += '\n'
        if self.print_detail:
            result_str += '----------------------- Details ------------------------\n'
            for n, r in zip(self.rel_name_list, self.result_dict[mode + '_mean_recall_zs_list'][100]):
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + '_mean_recall_zs_collect']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            if len(self.zeroshot_idx) > 0:
                if not isinstance(match, (list, tuple)):
                    ...
                    match_list = match
                zeroshot_match = list(set(self.zeroshot_idx).intersection(set(match_list)))
                recall_hit = [0] * self.num_rel
                recall_count = [0] * self.num_rel
                for idx in range(len(self.zeroshot_idx)):
                    local_label = gt_rels[int(self.zeroshot_idx[idx]), 2]
                    recall_count[int(local_label)] += 1
                    recall_count[0] += 1
                for idx in range(len(zeroshot_match)):
                    local_label = gt_rels[int(zeroshot_match[idx]), 2]
                    recall_hit[int(local_label)] += 1
                    recall_hit[0] += 1
                for n in range(self.num_rel):
                    if recall_count[n] > 0:
                        self.result_dict[mode + '_mean_recall_zs_collect'][k][n].append(float(recall_hit[n] / recall_count[n]))
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_mean_zs_recall'].items():
            ...
            for idx in range(num_rel_no_bg):
                if len(self.result_dict[mode + '_mean_recall_zs_collect'][k][idx+1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(self.result_dict[mode + '_mean_recall_zs_collect'][k][idx+1])
                self.result_dict[mode + '_mean_recall_zs_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode + '_mean_zs_recall'][k] = sum_recall / float(num_rel_no_bg)
        return
...
def rel_nms(pred_boxes, pred_classes, pred_rel_inds, rel_scores, overall_scores, nms_thresh=0.5, l21_thr=0.7):
...
def rel_nms_ng(pred_boxes, pred_classes, nogc_pred_rels, nms_thresh=0.5):
    ...
    is_overlap = (rel_ious>=nms_thresh)&(sub_labels[:,None]==sub_labels[None,:])&(obj_labels[:,None]==obj_labels[None,:])&(rel_labels[:, None]==rel_labels[None, :])
