...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGZeroShotMeanRecall
...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load("./zeroshot_triplet_new.pytorch", map_location=torch.device("cpu")).long().numpy()
    ...
    if "relations" in iou_types:
        ...
        eval_zs_mean_recall = SGZeroShotMeanRecall(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        ...
        for img_id, (groundtruth, prediction) in tqdm(enumerate(zip(groundtruths, predictions))):
            evaluate_relation_of_one_image(img_id, groundtruth, prediction, global_container, evaluator)
        ...
        # print result
        # --change--
        # add_str, match_list = eval_recall.generate_print_string(mode)
        r_str, r_data = eval_recall.generate_print_string(mode)
        result_str += r_str
        torch.save(r_data, os.path.join(cfg.OUTPUT_DIR, 'recall_result.pytorch'))
        # torch.save(match_list, os.path.join(cfg.OUTPUT_DIR, 'recall.pytorch'))
        result_str += eval_nog_recall.generate_print_string(mode)
        # result_str += eval_zeroshot_recall.generate_print_string(mode)
        add_str, match_list = eval_zeroshot_recall.generate_print_string(mode)
        result_str += add_str
        torch.save(match_list, os.path.join(cfg.OUTPUT_DIR, 'zs.pytorch'))
...
def evaluate_relation_of_one_image(img_id, groundtruth, prediction, global_container, evaluator):
    """
    Returns:
    ...
    evaluator['eval_zs_mean_recall'].prepare_zeroshot(global_container, local_container)
    ...
    # --change--
    evaluator['eval_mean_micro_recall'].collect_mean_recall_items(global_container, local_container, mode)

    # Zero shot Recall
    evaluator['eval_zeroshot_recall'].calculate_recall(img_id, global_container, local_container, mode)
    ...
    evaluator['eval_zs_mean_recall'].collect_mean_recall_items(global_container, local_container, mode)
