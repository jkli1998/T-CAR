...
class RelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            non_bg_pair_idxs = torch.nonzero(tgt_rel_matrix != 0)
            non_bg_head_idxs = non_bg_pair_idxs[:, 0].contiguous().view(-1)
            non_bg_tail_idxs = non_bg_pair_idxs[:, 1].contiguous().view(-1)
            rel_possibility[non_bg_head_idxs, non_bg_tail_idxs] = 0
